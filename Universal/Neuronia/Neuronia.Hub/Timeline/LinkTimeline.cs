...
{
    [DataContract]
    public class LinkTimeline : TimelineBase
    {

        public LinkTimeline(TwitterAccount account, string listTitle, string tabName, SettingData setting,
            Action<TimelineAction> timelineActionCallback, Action<RowAction> rowActionCallback)
        ...
        public override async void Initialize(TwitterAccount account, SettingData setting,
            Action<TimelineAction> timelineActionCallback, Action<RowAction> rowActionCallback)
        {
            ...
            if (TimeLine.Count > 0)
            {
                ...
                var rows =
                    (await Account.TwitterClient.GetHomeTimelineNewAsync(sinceId)).Select(
                        r => new TimelineRow(r, Account.UserInfomation.screen_name, Setting, rowActionCallback))
                        .Cast<RowBase>()
                        .ToList();
                rows =
                    rows.Where(q => q.Tweet.entities.urls != null && q.Tweet.entities.urls.Count > 0)
                        .Select(q => q)
                        .ToList();
            ...
            {
                var rows =
                    (await Account.TwitterClient.GetHomeTimelineAsync(200)).Select(
                        r => new TimelineRow(r, Account.UserInfomation.screen_name, Setting, rowActionCallback))
                        .Cast<RowBase>()
                        .ToList();
                rows =
